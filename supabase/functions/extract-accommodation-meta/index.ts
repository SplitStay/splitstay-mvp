...
async function extractWithPuppeteer(
  ...
  // biome-ignore lint/suspicious/noImplicitAnyLet: Puppeteer browser type inference
  let browser;
  try {
    ...
    if (parsedUrl.hostname.includes('booking.com')) {
      try {
        await page.waitForSelector(
          'img[data-testid="hero-image"], .bh-photo-grid img, .hp__gallery-container img',
          { timeout: 5000 },
        );
      } catch (_e) {
    ...
    const extractedData = await page.evaluate((hostname) => {
      if (hostname.includes('booking.com')) {
        return {
          title:
            document
              .querySelector('h2[data-testid="header-title"]')
              ?.textContent?.trim() ||
          ...
          image:
            document
              .querySelector('img[data-testid="hero-image"]')
              ?.getAttribute('src') ||
            document.querySelector('.bh-photo-grid img')?.getAttribute('src') ||
            document
              .querySelector('.hotel_header_image img')
              ?.getAttribute('src') ||
            document
              .querySelector('img[data-testid="property-gallery-image"]')
              ?.getAttribute('src') ||
            document
              .querySelector('.hp__gallery-container img')
              ?.getAttribute('src') ||
            document
              .querySelector('[data-testid="gallery-images"] img')
              ?.getAttribute('src') ||
            document
              .querySelector('.hp-gallery__image img')
              ?.getAttribute('src') ||
            document.querySelector('.gallery-image img')?.getAttribute('src') ||
            document
              .querySelector('img[alt*="hotel" i]')
              ?.getAttribute('src') ||
            document
              .querySelector('img[alt*="property" i]')
              ?.getAttribute('src') ||
            document
              .querySelector('img[src*="booking.com"]')
              ?.getAttribute('src'),

          description:
            document
              .querySelector('[data-testid="property-description"]')
              ?.textContent?.trim() ||
            document
              .querySelector('.hotel_description_wrapper_exp')
              ?.textContent?.trim() ||
            document
              .querySelector('.hp_desc_main_content')
              ?.textContent?.trim(),
        ...
        return {
          title:
            document
              .querySelector('h1[data-stid="content-hotel-title"]')
              ?.textContent?.trim() ||
            document.querySelector('h1')?.textContent?.trim(),
          image:
            document
              .querySelector('img[data-stid="hero-image"]')
              ?.getAttribute('src') ||
            document.querySelector('.hero-image img')?.getAttribute('src'),
          description: document
            .querySelector('[data-stid="content-hotel-description"]')
            ?.textContent?.trim(),
          site: 'Hotels.com',
        };
      } else {
      ...
      JSON.stringify({
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to extract with browser',
      }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
...
serve(async (req) => {
  ...
  try {
    ...
    const needsPuppeteer = [
      ...
      'expedia.com',
      'agoda.com',
    ].some((site) => parsedUrl.hostname.includes(site));
