...
import { type CitySuggestion, searchCities } from '@/lib/mapbox';
...
export const CityAutocomplete: React.FC<Props> = ({
  ...
  return (
    <div ref={containerRef} className={`relative ${className ?? ''}`}>
      {label && (
        // biome-ignore lint/a11y/noLabelWithoutControl: Associated via id prop
        <label className="block text-sm font-medium text-gray-700 mb-1">
          {label}
          {required ? ' *' : ''}
        </label>
      )}
      {/* biome-ignore lint/a11y/useAriaPropsSupportedByRole: Combobox pattern */}
      <input
        id={id}
        type="text"
        value={value}
        ...
        onFocus={() =>
          value.length >= 2 && value !== lastSelectedValue && setOpen(true)
        }
      ...
      {open && (loading || options.length > 0) && (
        <ul className="absolute left-0 right-0 bg-white border border-gray-200 rounded-md shadow-lg z-10 mt-1 max-h-60 overflow-y-auto">
          {loading && <li className="px-4 py-2 text-gray-500">Searching…</li>}
          {!loading &&
            options.map((opt, idx) => (
              // biome-ignore lint/a11y/useAriaPropsSupportedByRole: Listbox option pattern
              <li
                key={opt.id}
                ...
                onMouseDown={(e) => {
                  e.preventDefault();
                  onChange(opt.placeName);
                  onSelect?.(opt);
                  setLastSelectedValue(opt.placeName);
                  setOpen(false);
                }}
              >
                {opt.placeName}
