...
export const MessagesPage: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      const conversationsSub = ChatService.subscribeToUserConversations(user.id, (conv: Conversation) => {
        setConversations(prev => {
          const idx = prev.findIndex(c => c.id === conv.id)
          const mapped: ConversationWithUser = idx !== -1 ? { ...prev[idx] } : {
            ...(conv as any),
            other_user: prev[idx]?.other_user || (prev[0]?.other_user ?? { id: '', name: '', email: '' }),
          ...
          const next = idx !== -1 ? [...prev.slice(0, idx), mapped, ...prev.slice(idx + 1)] : [mapped, ...prev]
          return next
            .slice()
      ...
      const loadPresenceForConversations = async (partnerIds: string[]) => {
        ...
        if (data) {
          ...
          for (const row of data as any[]) {
            const last = row.last_active_at ? new Date(row.last_active_at) : new Date(0)
          ...
          {selectedConversation ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
              ...
              <p className="text-gray-500 max-w-md mb-6">
                Choose a conversation from the sidebar to start messaging, or find new travel partners to chat with.
              </p>
              <button
