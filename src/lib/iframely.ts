...
class IframelyService {
  ...
  private readonly apiKey =
    import.meta.env.VITE_IFRAMELY_API_KEY || 'e7ef2fd691282a8112c2ce56874a1d24';
  ...
  private createBookingSiteFallback(url: string): AccommodationPreview {
    ...
    const title =
      hotelName ||
      `${siteName.charAt(0).toUpperCase() + siteName.slice(1)} Accommodation`;
  ...
  async getAccommodationPreview(
    ...
    try {
      ...
      if (isBlockedBookingSite && this.supabaseUrl) {
        console.log(
          'Attempting server-side extraction for blocked booking site:',
          url,
        );
        try {
          ...
          if (serverResult.success) {
            ...
            if (!title && url.includes('booking.com')) {
              ...
              const hotelSlug = urlParts.find(
                (part) => part.includes('hotel') || part.length > 10,
              );
              if (hotelSlug) {
                title = hotelSlug
                  .replace(/hotel-|hotel\//, '')
          ...
          console.log(
            'Server-side extraction failed, falling back to Iframely:',
            serverError,
          );
        }
      }
      ...
      if (!this.apiKey) {
        return {
          ...
          description:
            'This is a demo preview. Configure VITE_IFRAMELY_API_KEY to see real accommodation previews from Booking.com, Airbnb, and other sites.',
          image: '',
          site: 'Demo Site',
          author: '',
          url,
  ...
  private async extractMetaViaSupabase(url: string): Promise<{
    ...
    const response = await fetch(
      `${this.supabaseUrl}/functions/v1/extract-accommodation-meta`,
      {
        method: 'POST',
        headers: {
